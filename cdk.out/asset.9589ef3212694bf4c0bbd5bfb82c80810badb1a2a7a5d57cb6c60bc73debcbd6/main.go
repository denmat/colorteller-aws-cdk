...
func (h *colorHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	ct, err := net.LookupIP("colorteller.svc.test.local")
	bl, err := net.LookupIP("colorteller-black.svc.test.local")
	gw, err := net.LookupIP("gateway.svc.test.local")
	log.Printf("IP: colorteller: %s", ct)
	log.Printf("IP: black: %s", bl)
	log.Printf("IP: gateway: %s", gw)
